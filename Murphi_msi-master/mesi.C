...
class mu_1_MessageType: public mu__byte
{
 public:
  ...
  mu_1_MessageType (char *name, int os): mu__byte(6, 19, 4, name, os) {};
  mu_1_MessageType (void): mu__byte(6, 19, 4) {};
  mu_1_MessageType (int val): mu__byte(6, 19, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1_MessageType::values[] = {"GetS","GetM","PutS","PutE","PutM","InvAck","Data","DataE","FwdAck","TranAck","FwdGetS","FwdGetM","PutAck","Inv",NULL };
...
void mu_1_Message::set_self(char *n, int os)
{
  ...
  mu_dst.set_self_2(name, ".dst", os + 16 );
...
char *mu_1__type_0::values[] = {"H_I","H_S","H_E","H_M","H_MM_Wait","H_MS_Wait","H_SM_Wait","H_IE_Wait","H_IM_Wait",NULL };
...
char *mu_1__type_2::values[] = {"P_I","P_S","P_E","P_M","P_IS_D","P_IM_AD","P_IM_A","P_SM_AD","P_SM_A","P_SI_A","P_EI_A","P_MI_A","P_II_A",NULL };
...
void mu_1__type_5::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"HomeType", i * 224 + os);i++;
...
const int mu_DataE = 13;
const int mu_FwdAck = 14;
const int mu_TranAck = 15;
...
const int mu_H_E = 22;
const int mu_H_M = 23;
const int mu_H_MM_Wait = 24;
const int mu_H_MS_Wait = 25;
const int mu_H_SM_Wait = 26;
const int mu_H_IE_Wait = 27;
const int mu_H_IM_Wait = 28;
const int mu_P_I = 29;
const int mu_P_S = 30;
const int mu_P_E = 31;
const int mu_P_M = 32;
const int mu_P_IS_D = 33;
const int mu_P_IM_AD = 34;
const int mu_P_IM_A = 35;
const int mu_P_SM_AD = 36;
const int mu_P_SM_A = 37;
const int mu_P_SI_A = 38;
const int mu_P_EI_A = 39;
const int mu_P_MI_A = 40;
...
/*** Variable declaration ***/
mu_1__type_7 mu_InBox("InBox",1040);

/*** Variable declaration ***/
mu_0_boolean mu_msg_processed("msg_processed",1616);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",1624);
...
case mu_GetS:
if ( !((mu_cnt) == (0)) ) Error.Error("Assertion failed: H_I's cnt is not always zero");
mu_Send ( mu_DataE, mu_msg.mu_src, (int)mu_HomeType, mu_VC2, mu_HomeNode.mu_val, (int)mu_cnt );
mu_HomeNode.mu_owner = mu_msg.mu_src;
mu_HomeNode.mu_state = mu_H_IE_Wait;
break;
case mu_GetM:
...
case mu_PutE:
bool mu__boolexpr19;
  if (mu_IsSharer( mu_msg.mu_src )) mu__boolexpr19 = TRUE ;
  else {
  mu__boolexpr19 = ((mu_HomeNode.mu_owner) == (mu_msg.mu_src)) ; 
}
if ( mu__boolexpr19 )
{
mu_msg_processed = mu_false;
...
bool mu__boolexpr20;
  if ((mu_msg.mu_ack) == (0)) mu__boolexpr20 = TRUE ;
  else {
  mu__boolexpr20 = ((mu_pa) == (0)) ; 
}
if ( mu__boolexpr20 )
{
mu_Send ( mu_TranAck, (int)mu_HomeType, (int)mu_p, mu_VC2, mu_1_Value_undefined_var, mu_1_Acknowledge_undefined_var );
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("evict exclusive block, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_PutE, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_1_Value_undefined_var, mu_1_Acknowledge_undefined_var );
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("store request on E, v:%s, n:%s", mu_v.Name(), mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Value mu_v;
...
bool Condition(unsigned r)
{
  ...
  if (r>=46 && r<=48) return R8.Condition(r-46);
  if (r>=49 && r<=51) return R9.Condition(r-49);
...
void Code(unsigned r)
{
  ...
  if (r>=46 && r<=48) { R8.Code(r-46); return; } 
  if (r>=49 && r<=51) { R9.Code(r-49); return; } 
}
int Priority(unsigned short r)
{
  ...
  if (r>=46 && r<=48) { return R8.Priority(); } 
  if (r>=49 && r<=51) { return R9.Priority(); } 
}
char * Name(unsigned r)
{
  ...
  if (r>=46 && r<=48) return R8.Name(r-46);
  if (r>=49 && r<=51) return R9.Name(r-49);
  return NULL;
}
};
...
bool mu__boolexpr23;
bool mu__boolexpr24;
  if (!((mu_HomeNode.mu_state) == (mu_H_S))) mu__boolexpr24 = FALSE ;
  else {
  mu__boolexpr24 = ((mu_Procs[mu_n].mu_state) == (mu_P_S)) ; 
}
...
int mu__invariant_26() // Invariant "values in memory matches value of last write, when shared or invalid"
{
bool mu__quant27; 
mu__quant27 = TRUE;
{
...
bool mu__boolexpr28;
bool mu__boolexpr29;
  if ((mu_HomeNode.mu_state) == (mu_H_S)) mu__boolexpr29 = TRUE ;
  else {
  mu__boolexpr29 = ((mu_HomeNode.mu_state) == (mu_H_I)) ; 
}
...
bool mu__condition_30() // Condition for Rule "values in memory matches value of last write, when shared or invalid"
{
...
int mu__invariant_35() // Invariant "exclusive implies empty sharers list"
{
bool mu__boolexpr36;
  if (!((mu_HomeNode.mu_state) == (mu_H_E))) mu__boolexpr36 = TRUE ;
  else {
...
bool mu__condition_38() // Condition for Rule "exclusive implies empty sharers list"
{
...
{
bool mu__boolexpr40;
  if (!((mu_HomeNode.mu_state) == (mu_H_M))) mu__boolexpr40 = TRUE ;
  else {
...
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr45;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_I))) mu__boolexpr45 = TRUE ;
  else {
...
bool mu__boolexpr51;
  ...
  mu__boolexpr50 = ((mu_Procs[mu_n].mu_state) == (mu_P_E)) ; 
}
...
bool mu__boolexpr57;
  ...
  mu__boolexpr57 = (mu_HomeNode.mu_owner.isundefined()) ; 
}
...
{"Invalid implies empty owner", &mu__condition_58, NULL, FALSE},
...
{"exclusive implies empty sharers list", &mu__condition_38, NULL, FALSE},
{"Invalid implies empty sharer list", &mu__condition_34, NULL, FALSE},
{"values in memory matches value of last write, when shared or invalid", &mu__condition_30, NULL, FALSE},
